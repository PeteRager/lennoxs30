...
- Thermostat linked to a lennoxicomfort.com account, or a thermostat on the local LAN
- For local connections, the thermostat may be configured manually by IP/hostname or discovered automatically through mDNS. A static/DHCP-leased IP is still supported, but is not required when the thermostat advertises its mDNS service.

# Development

This repository includes a phased pre-commit configuration with basic file checks, Ruff formatting, and syntax-level linting on commit.

Install and enable it in your local clone:
...
Run the default (lightweight) hooks on all files any time with:

```bash
pre-commit run --all-files
```

Run strict Ruff linting manually when you are ready:
...
- Local Lennox S30/S40 thermostats that advertise `_icomfort4._res._lii._http._tcp.local.` can be discovered automatically by Home Assistant. Cloud connections are not discovered through mDNS.
...
Most will use the Local Connection. Local connections are faster and more reliable than Cloud Connections. You can enter an IP address or hostname manually. If Home Assistant discovers a compatible thermostat through mDNS, it also presents a separate discovery notification that you can configure without entering an address.
...
Use this section when adding a local thermostat manually, rather than selecting an mDNS discovery notification.
...
Home Assistant can automatically discover LAN-connected Lennox S30/S40 thermostats that advertise the `_icomfort4._res._lii._http._tcp.local.` service. Select the discovery notification to configure one without entering an address. mDNS discovery applies only to local connections; cloud connections are unchanged.

For existing local entries, discovery keeps the same config entry and entities while switching the connection to the thermostat’s hostname, allowing it to follow IP address changes.
