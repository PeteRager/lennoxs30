...
@pytest.mark.asyncio
async def test_async_setup_entry_local(hass, caplog):
    data = {
    ...
    with patch.object(hass.bus, "async_listen_once") as listen_once:
        with patch("custom_components.lennoxs30.Manager.s30_initialize") as s30_initialize:
            ...
            assert listen_once.call_count == 1
            assert listen_once.call_args[0][0] == EVENT_HOMEASSISTANT_STOP
            assert listen_once.call_args[0][1] == manager.async_shutdown
...
async def test_async_unload_entry_success(hass, caplog):
    ...
    with patch.object(hass.bus, "async_listen_once") as _:
